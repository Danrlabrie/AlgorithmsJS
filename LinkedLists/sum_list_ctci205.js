...
// so we can basically work our way through both lists, summing to just a number value and then break that into
...
let sumLists = (ll1, ll2) => {
  ...
  while(current1 || current2) {
    let first = !!current1.data ? current1.data : 0;
    let second = !!current1.data ? current1.data : 0;
...
// we would really need to know the length of the lists in order to pad out the front of the shorter one
...
let sumListsForward = (ll1, ll2) => {
 ...
 reverse1 = new LinkedList();
 reverse2 = new LinkedList();
 ...
 return sumLists(revese1, reverse2);
};
